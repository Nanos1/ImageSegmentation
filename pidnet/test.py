...
def test(BATCH_SIZE):
	...
	model.load_weights('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/pidnet/weights').expect_partial()
	...
	if not os.path.exists('pidnet/figures'):
	    os.makedirs('pidnet/figures')
	r = 0
	for image, mask in test_dataset.take(batch_size):
	  ...
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('PIDNet Prediction')
	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('pidnet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))
