...
def test(BATCH_SIZE):
	...
	DATA_DIR_TEST = '/work2/pa21/ptzouv/gnanos/kvasir_inst_dataset/dataset_256/test/'
	test_img_list = glob("/work2/pa21/ptzouv/gnanos/kvasir_inst_dataset/dataset_256/test/images/*")
	test_mask_list = glob('/work2/pa21/ptzouv/gnanos/kvasir_inst_dataset/dataset_256/test/masks/*')
	...
	model.load_weights('/work2/pa21/ptzouv/gnanos/kvasir_inst_dataset/images_256/basnet/weights').expect_partial()
	...
	print('-'*50)
#       print("%s: %.5f%%" % (model.metrics_names[2], result[2]*100))
	for i in range(1, len(model.metrics_names)):
	  print("%s: %.5f%%\n" % (model.metrics_names[i], result[i]*100))
		
	#=== create some figures
	plt.rcParams.update({'figure.max_open_warning': 0})
	if not os.path.exists('basnet/figures'):
	    os.makedirs('basnet/figures')
	r = 0
	for image, mask in test_dataset.take(16):
	  r +=1
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('BASNET Prediction')
#	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')
	    ax3.imshow(pred_mask_h[0][0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('basnet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))
