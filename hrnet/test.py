...
weights_list = glob('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/hrnet/model_save/*')
def test(BATCH_SIZE, weight):
	...
	model = tf.keras.models.load_model(weight, custom_objects={"iou_loss": iou_loss, "recall2": recall2, "recall3": recall3, "precision2": precision2,"precision3": precision3, "dice_loss": dice_loss, "dice_loss2": dice_loss2})
	...
	if not os.path.exists('hrnet/figures'):
	    os.makedirs('hrnet/figures')

	r = 0
	for image, mask in test_dataset.take(batch_size):
	  r +=1
	  for i in range(batch_size):
	    ...
	    plt.savefig(os.path.join('hrnet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))

	return(result[1], result[4], result[6], result[9])


test(16, '/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/hrnet/model_save/weights')
