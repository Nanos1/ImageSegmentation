...
weights_list = glob('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/bisnet/model_save/*')
...
def test(BATCH_SIZE, weight):
	...
	if not os.path.exists('bisnet/figures'):
		os.makedirs('bisnet/figures')
	r = 0
	for image, mask in test_dataset.take(batch_size):
	  ...
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('BISNET Prediction')
	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('bisnet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))

	return(result[1], result[4], result[6], result[10])

test(16, '/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/bisnet/model_save/weights')
