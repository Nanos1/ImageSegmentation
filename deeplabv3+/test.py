...
weights_list = glob('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/deeplabv3+/model_save/*')
...
def test(BATCH_SIZE, weight):
	...
	model.load_weights(weight)#.expect_partial()  260
	...
	if not os.path.exists('deeplabv3+/figures'):
	    os.makedirs('deeplabv3+/figures')
	r = 0
	for image, mask in test_dataset.take(batch_size):
	  ...
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('DeepLabv3plus Prediction')
	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('deeplabv3+/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))


	return(result[1], result[4], result[6], result[9])

test(16, '/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/deeplabv3+/weights')
