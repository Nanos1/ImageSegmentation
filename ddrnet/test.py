...
OUTPUT_CHANNELS = 1
weights_list = glob('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/ddrnet/model_save/*')
...
def test(BATCH_SIZE, weight):
	...
	if not os.path.exists('ddrnet/figures'):
		os.makedirs('ddrnet/figures')
	...
	for image, mask in test_dataset.take(batch_size):
	  ...
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('DDRNET Prediction')
	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('ddrnet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))
	
	return(result[1], result[4], result[6], result[9])

test(64, '/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/ddrnet/weights')
