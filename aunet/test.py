...
weights_list = glob('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/aunet/model_save/*')
def test(BATCH_SIZE, weight):
	...
	attention_unet_model = Attention_UNet(input_shape=input_shape)
	model = attention_unet_model.build_attention_unit()
	...
	if not os.path.exists('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/aunet/figures'):
	    os.makedirs('/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/aunet/figures')
	r = 0
	for image, mask in test_dataset.take(2):
	  ...
	  for i in range(batch_size):
	    ...
	    ax3.title.set_text('UNET3+ Prediction')
	    ax3.imshow(pred_mask_h[0,:,:,0], cmap='gray')

	    plt.savefig(os.path.join('aunet/figures', 'prediction_fig_' + str(i)+ '_' + str(r) + '.png'))
	
	return(result[1], result[4], result[6], result[9])

test(16, '/work2/pa21/ptzouv/gnanos/images_256/monitor_val_loss/aunet/model_save/weights')
